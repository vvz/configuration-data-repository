...
//        'org.jsecurity.spring.LifecycleBeanPostProcessor'(org.jsecurity.spring.LifecycleBeanPostProcessor)
    ...
    def doWithDynamicMethods = { ctx ->
        ...
        def controlCheck = { Closure c ->
            ...
            def filtersClass = delegate.filtersDefinition
            ...
            if (!subject.authenticated) {
                ...
                if (filtersClass.metaClass.respondsTo(filtersClass, 'onNotAuthenticated')) {
                    filtersClass.onNotAuthenticated(subject, delegate)
                }
                else {
            ...
            if (c == null) {
                ...
                // by convention.
                isPermitted = subject.isPermitted(new JsecBasicPermission(controllerName, actionName ?: 'index'))
            }
            else {
            ...
            if (!isPermitted) {
                ...
                if (filtersClass.metaClass.respondsTo(filtersClass, 'onUnauthorized')) {
                    filtersClass.onUnauthorized(subject, delegate)
                }
                else {
        ...
        mc.accessControl = controlCheck.curry(null)
